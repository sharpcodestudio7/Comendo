...
// Panel lateral del carrito. Muestra los items, subtotal y botón de confirmar pedido.

import useCartStore from '../store/useCartStore';

const CartDrawer = ({ abierto, onCerrar }) => {
  ...
  const handleConfirmar = () => {
    // Por ahora solo muestra un alert — aquí conectaremos Supabase en el siguiente paso
    alert('¡Pedido confirmado! Pronto lo conectaremos a la cocina.');
    limpiarCarrito();
    onCerrar();
  };
  ...
  return (
    <>
      ...
      <div style={styles.drawer}>
              ...
              <div key={producto.id_producto} style={styles.item}>
                ...
                <div style={styles.itemControles}>
                  <button
                    ...
                    onClick={() => quitarItem(producto.id_producto)}
                  >−</button>
                  ...
                  <button
                    ...
                    onClick={() => eliminarItem(producto.id_producto)}
                  >🗑</button>
                </div>
        ...
        {/* Footer con totales y botón confirmar */}
        {items.length > 0 && (
          <div style={styles.footer}>
            ...
            <div style={styles.filaTotal}>
              <span>IVA (19%)</span>
              <span>{formatearPrecio(subtotal() * 0.19)}</span>
            </div>
            <div style={{ ...styles.filaTotal, ...styles.filaTotalFinal }}>
              <span>Total</span>
              <span>{formatearPrecio(subtotal() * 1.19)}</span>
            </div>
...
const styles = {
  ...
  btnContador: {
    ...
    color: '#2e7d32', fontSize: '18px', cursor: 'pointer', fontWeight: '700',
  },
  cantidad: { fontSize: '16px', fontWeight: '700', minWidth: '20px', textAlign: 'center' },
  btnEliminar: {
    background: 'none', border: 'none',
    fontSize: '16px', cursor: 'pointer', marginLeft: 'auto',
  },
  footer: {
    padding: '16px',
